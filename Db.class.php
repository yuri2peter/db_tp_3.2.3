    ...
    static private function parseConfig($config){
        if(!empty($config)){
            ...
            $config = array (
                ...
                'debug'         =>  isset($config['db_debug'])?$config['db_debug']:Adapter::isDebugMode(),
                'lite'          =>  isset($config['db_lite'])?$config['db_lite']:false,
            );
        }else {
            $config = array (
                'type'          =>  Adapter::C('DB_TYPE'),
                'username'      =>  Adapter::C('DB_USER'),
                'password'      =>  Adapter::C('DB_PWD'),
                'hostname'      =>  Adapter::C('DB_HOST'),
                'hostport'      =>  Adapter::C('DB_PORT'),
                'database'      =>  Adapter::C('DB_NAME'),
                'dsn'           =>  Adapter::C('DB_DSN'),
                'params'        =>  Adapter::C('DB_PARAMS'),
                'charset'       =>  Adapter::C('DB_CHARSET'),
                'deploy'        =>  Adapter::C('DB_DEPLOY_TYPE'),
                'rw_separate'   =>  Adapter::C('DB_RW_SEPARATE'),
                'master_num'    =>  Adapter::C('DB_MASTER_NUM'),
                'slave_no'      =>  Adapter::C('DB_SLAVE_NO'),
                'debug'         =>  Adapter::C('DB_DEBUG',Adapter::isDebugMode()),
                'lite'          =>  Adapter::C('DB_LITE'),
            );
        }
        return $config;
    }
