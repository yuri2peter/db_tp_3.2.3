...
class Mongo extends Driver {
    ...
    public function command($command=array(), $options=array()) {
        ...
        if($cache) { // 查询缓存检测
            ...
            $value   =  Adapter::S($key,'','',$cache['type']);
            if(false !== $value) {
                return $value;
            }
        }
        ...
        try{
            ...
            if($cache && $result['ok']) { // 查询缓存写入
                Adapter::S($key,$result,$cache['expire'],$cache['type']);
            }
            return $result;
    ...
    public function group($keys,$initial,$reduce,$options=array()){
        ...
        try{
            ...
            if($cache && $group['ok'])
                Adapter::S($key,$group,$cache['expire'],$cache['type']);
