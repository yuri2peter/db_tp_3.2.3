...
class MergeModel extends Model {
    ...
    public function __construct($name='',$tablePrefix='',$connection=''){
        ...
        if(empty($this->fields) && !empty($this->modelList)){
            ...
            foreach($this->modelList as $model){
                // 获取模型的字段信息
                $result     =   $this->db->getFields(Adapter::M($model,empty($this->tablePrefix)?'':$this->tablePrefix,empty($this->connection)?'':$this->connection)->getTableName());
        ...
        // 主表的主键名
        $this->pk =   Adapter::M($this->masterModel,empty($this->tablePrefix)?'':$this->tablePrefix,empty($this->connection)?'':$this->connection)->getPk();
    ...
    public function getTableName() {
        if(empty($this->trueTableName)) {
            ...
            foreach($models as $model){
                $tableName[]    =   Adapter::M($model,empty($this->tablePrefix)?'':$this->tablePrefix,empty($this->connection)?'':$this->connection)->getTableName().' '.$model;
            }
    ...
    public function add($data='',$options=array(),$replace=false){
        ...
        $this->startTrans();
        // 写入主表数据
        $result     =   Adapter::M($this->masterModel,empty($this->tablePrefix)?'':$this->tablePrefix,empty($this->connection)?'':$this->connection)->strict(false)->add($data);
        if($result){
            // 写入外键数据
            $data[$this->fk]    =   $result;
            $models     =   $this->modelList;
            ...
            // 写入附表数据
            foreach($models as $model){
                $res =   Adapter::M($model)->strict(false)->add($data);
                if(!$res){
                    $this->rollback();
                    return false;
                }
   ...
   /**
    ...
    protected function _options_filter(&$options) {
        ...
        $options['table']   =   Adapter::M($this->masterModel)->getTableName().' '.$this->masterModel;
