...
class ViewModel extends Model {
        ...
        if(empty($this->trueTableName)) {
            ...
            foreach ($this->viewFields as $key=>$view){
                ...
                if(isset($view['_table'])) { // 2011/10/17 添加实际表名定义支持 可以实现同一个表的视图
                    ...
                    $class  =   $key.'Model';
                    $Model  =  class_exists($class)?new $class(): Adapter::M($key,empty($this->tablePrefix)?'':$this->tablePrefix,empty($this->connection)?'':$this->connection);
    ...
    private function _checkFields($name,$fields) {
        if(false !== $pos = array_search('*',$fields)) {// 定义所有字段
            $fields  =  array_merge($fields, Adapter::M($name,empty($this->tablePrefix)?'':$this->tablePrefix,empty($this->connection)?'':$this->connection)->getDbFields());
            unset($fields[$pos]);
        }
        return $fields;
    }
