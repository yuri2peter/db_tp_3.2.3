        ...
        if(!empty($this->_link)) {
            foreach($this->_link as $key=>$val) {
                    ...
                    if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name))) {
                        ...
                        // 获取关联模型对象
                        $model = Adapter::D($mappingClass);
                        if (!$model){
                            $model=Adapter::M($name,empty($this->tablePrefix)?'':$this->tablePrefix,empty($this->connection)?'':$this->connection);
                        }
                        switch($mappingType) {
                            case self::HAS_ONE:
        ...
        if(!empty($this->_link)) {
            ...
            foreach($this->_link as $key=>$val) {
                    ...
                    if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name)) ) {
                        ...
                        $model =  Adapter::D($mappingClass);
                        if (!$model){
                            $model=Adapter::M($mappingClass,empty($this->tablePrefix)?'':$this->tablePrefix,empty($this->connection)?'':$this->connection);
                        }
