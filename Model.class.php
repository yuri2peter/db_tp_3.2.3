 ...
 * @method $this order(string|array $order_by) order(array('order','id'=>'desc')) or order('id desc,status')
 * @method $this alias(string $table_alias)
 * @method $this group(string $group_by)
 * @method $this having(string $condition)
 * @method $this distinct(bool $enable)
 * @method $this lock(bool $enable)
 * @method $this strict(bool $enable)
 * @method $this relation(bool|string $enable)
 * @method $this relationGet(bool|string $enable) 在查询后再进行关联查询
 * @method $this index(string $index) index方法用于数据集的强制索引操作
 */
class Model {
    // 操作状态
    ...
    // 链操作方法列表
    protected $methods          =   array('strict','order','alias','having','group','lock','distinct','auto','filter','validate','result','index','force');
    ...
    public function __construct($name='',$tablePrefix='',$connection='') {
        ...
        if(is_null($tablePrefix)) {// 前缀为Null表示没有前缀
            ...
            $this->tablePrefix = Adapter::C('DB_PREFIX');
        }
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            // 如果数据表字段没有定义则自动获取
            if(Adapter::C('DB_FIELDS_CACHE')) {
                $db   =  $this->dbName?:Adapter::C('DB_NAME');
                $fields = Adapter::F('_fields/'.strtolower($db.'.'.$this->tablePrefix.$this->name));
                if($fields) {
                    $this->fields   =   $fields;
                    if(!empty($fields['_pk'])){
                        $this->pk       =   $fields['_pk'];
    ...
    public function flush() {
        ...
        if(Adapter::C('DB_FIELDS_CACHE')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?:Adapter::C('DB_NAME');
            Adapter::F('_fields/'.strtolower($db.'.'.$this->tablePrefix.$this->name),$this->fields);
        }
    }
    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),$this->methods,true)) {
            ...
            // 根据某个字段获取记录
            $field   =   Adapter::parse_name(substr($method,5));
            $where[$field] =  $args[0];
            return $this->where($where)->find();
        }elseif(strtolower(substr($method,0,10))=='getfieldby') {
            // 根据某个字段获取记录的某个值
            $name   =   Adapter::parse_name(substr($method,10));
            $where[$name] =$args[0];
            return $this->where($where)->getField($args[1]);
        }elseif(isset($this->_scope[$method])){// 命名范围的单独调用支持
            return $this->scope($method,$args[0]);
        }else{
            Adapter::E(__CLASS__.':'.$method.' method not exist');
            return;
        }
    }
    // 回调方法 初始化模型
            ...
            foreach ($data as $key=>$val){
                if(!in_array($key,$fields,true)){
                    if(!empty($this->options['strict'])){
                        Adapter::E('data type invalid:['.$key.'=>'.$val.']');
                    }
                    unset($data[$key]);
        ...
        if(isset($options['where']) && is_array($options['where']) && !empty($fields) && !isset($options['join'])) {
            ...
            foreach ($options['where'] as $key=>$val){
                ...
                if(in_array($key,$fields,true)){
                    ...
                    if(!empty($this->options['strict'])){
                        Adapter::E('error query express'.':['.$key.'=>'.$val.']');
                    } 
    ...
    protected function _read_data($data) {
        // 检查字段映射
        if(!empty($this->_map) && Adapter::C('DB_READ_DATA_MAP')) {
    ...
    protected function returnResult($data,$type=''){
        if ($type){
            ...
            switch (strtolower($type)){
                ...
                case 'xml':
                    return Adapter::xml_encode($data);
            }
        }
        return $data;
    }
    ...
    protected function lazyWrite($guid,$step,$lazyTime) {
        if(false !== ($value = Adapter::S($guid))) { // 存在缓存写入数据
            if(Adapter::getNowTime() > Adapter::S($guid.'_time')+$lazyTime) {
                // 延时更新时间到了，删除缓存数据 并实际写入数据库
                Adapter::S($guid,NULL);
                Adapter::S($guid.'_time',NULL);
                return $value+$step;
            }else{
                // 追加数据到缓存
                Adapter::S($guid,$value+$step);
                return false;
            }
        }else{ // 没有缓存数据
            Adapter::S($guid,$step);
            // 计时开始
            Adapter::S($guid.'_time',Adapter::getNowTime());
            return false;
        }
    }
...
//        }
        ...
        if($this->autoCheckFields) { // 开启字段检测 则过滤非法字段数据
            ...
            foreach ($data as $key=>$val){
                if(!in_array($key,$fields)) {
                    unset($data[$key]);
                }elseif(Adapter::isMagic_Quotes_GPC() && is_string($val)){
    ...
    public function check($value,$rule,$type='regex'){
        ...
        switch($type) {
            ...
            case 'expire':
                ...
                return Adapter::getNowTime() >= $start && Adapter::getNowTime() <= $end;
            case 'ip_allow': // IP 操作许可验证
                return in_array(Adapter::getClientIp(),explode(',',$rule));
    ...
    public function getTableName() {
        if(empty($this->trueTableName)) {
            ...
            if(!empty($this->tableName)) {
                ...
                $tableName .= Adapter::parse_name($this->name);
            }
