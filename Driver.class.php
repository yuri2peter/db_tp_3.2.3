...
abstract class Driver
{
     ...
     * 架构函数 读取数据库配置信息
     * @access public
     * @param array|null $config 数据库配置数组
     */
    ...
    public function connect($config = '', $linkNum = 0, $autoConnection = false)
    {
        if (!isset($this->linkID[$linkNum])) {
            ...
            try {
                ...
                if ($autoConnection) {
                    Adapter::trace($e->getMessage(), '', 'ERR');
                    return $this->connect($autoConnection, $linkNum);
                } elseif ($config['debug']) {
                    Adapter::E($e->getMessage());
                }
            }
        }
    ...
    public function error()
    {
        ...
        // 记录错误日志
        Adapter::trace($this->error, '', 'ERR');
        if ($this->config['debug']) {// 开启数据库调试模式
            Adapter::E($this->error);
        } else {
            return $this->error;
        }
    }
        ...
        if (is_array($val)) {
            if (is_string($val[0])) {
                ...
                if (preg_match('/^(eq|neq|gt|egt|lt|elt)$/', $exp)) { // 比较运算
                    ...
                    Adapter::E('Expression errors :' . $val[0]);
                }
            } else {
                $count = count($val);
    ...
    protected function debug($start)
    {
        if ($this->config['debug']) {// 开启数据库调试模式
            if ($start) {
                Adapter::G('queryStartTime');
            } else {
                $this->modelSql[$this->model] = $this->queryStr;
                //$this->model  =   '_think_';
                // 记录操作结束时间
                Adapter::G('queryEndTime');
                Adapter::trace($this->queryStr . ' [ RunTime:' . Adapter::G('queryStartTime', 'queryEndTime') . 's ]', '', 'SQL');
            }
        }
    }
